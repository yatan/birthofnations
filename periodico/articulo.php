...
if ($sql == false)
    ...
    $megustavv = mysqli_query($link, "SELECT * FROM articulos_votos WHERE id_articulo=$id_articulo AND id_votador=$id_usuario");
    if (mysqli_num_rows($megustavv) > 0) {
        $megustav = mysqli_fetch_array($megustavv);
        if ($megustav['tipo'] == 1) {
?>
            <script>
                $(function() {
                    $("#dialog").dialog({
                        ...
                        autoOpen: false,
                        buttons: [{
                            text: "<?php echo getString('aceptar'); ?>",
                            click: function() {
                                $(this).dialog("close");
            ...
            <script>
                $(document).ready(function() {
                    ...
                    $("#yanomegusta").click(function() {
                        $.post("/periodico/gusta.php?doing=3&articulo_id=<?php echo $sql['id_articulo']; ?>",
                            function(data) {
                                $("#dialog").empty();
                                $("#dialog").append(data);
                                $("#dialog").dialog('open');
        ...
        <?php
            echo "<button id='yanomegusta' style='background-color:#1E679A ; border: 1px solid #1E679A;' href='#'>
                    Ya no me gusta
                </button>";
        } else {
        ?>
        ...
        <script>
            $(document).ready(function() {
                ...
                $("#megusta").click(function() {
                    $.post("/periodico/gusta.php?doing=1&articulo_id=<?php echo $sql['id_articulo']; ?>",
                        function(data) {
                            $("#dialogm").empty();
                            $("#dialogm").append(data);
                            $("#dialogm").dialog('open');
        ...
        <script>
            $(document).ready(function() {
                ...
                $("#nomegusta").click(function() {
                    $.post("/periodico/gusta.php?doing=2&articulo_id=<?php echo $sql['id_articulo']; ?>",
                        function(data) {
                            $("#dialogn").empty();
                            $("#dialogn").append(data);
                            $("#dialogn").dialog('open');
...
<script type="text/javascript">
    $('.enviar_comentario').click(function() {
        ...
        var notice = $.pnotify({
            pnotify_title: "<?php echo getstring('publicar_comentario'); ?>",
            pnotify_type: 'info',
