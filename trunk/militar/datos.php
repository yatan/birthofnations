...
$top_a = sql("SELECT id_usuario, SUM(hit) FROM log_hits WHERE id_guerra = 1 AND lado='a' GROUP BY id_usuario ORDER BY SUM(hit) DESC LIMIT 1");
$top_a_nick = id2nick($top_a['id_usuario']);
$top_a_dano = $top_a['SUM(hit)'];

$top_d = sql("SELECT id_usuario, SUM(hit) FROM log_hits WHERE id_guerra = 1 AND lado='d' GROUP BY id_usuario ORDER BY SUM(hit) DESC LIMIT 1");
